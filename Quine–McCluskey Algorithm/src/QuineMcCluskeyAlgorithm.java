...
public class QuineMcCluskeyAlgorithm {
    ...
    public static void main(String[] args) {
        ...
        try (PrintStream out = new PrintStream(new FileOutputStream(file,true))) {
            ...
            out.print("Total execution time: " + (double)(endTime - startTime)/1000000 + " mili.Sec \n");
            out.println("****************************************************");
            out.close();
    ...
    public static String[][] fill_minterms(String[] input_min) {
        String temp_min;
        int a = 0, count;
        int ones = 0, twos = 0, threes = 0, fours = 0, fives = 0, sixes = 0, sevens = 0, eights = 0, nines = 0, tens
                = 0, elevens = 0, twelves = 0, thirteens = 0;
                ...
                while (group_wise[min_comp][min_comp1] != "-1") {
                    ...
                    while (group_wise[min_comp2][min_comp3] != "-1") {
                        ...
                        for (int ij = 0; ij < bits; ij++) {
                            ...
                            if (group_wise[min_comp][min_comp1].charAt(ij) != group_wise[min_comp2][min_comp3].charAt
                                    (ij)) {
                                count++;
                                index = ij;
                            }
                        ...
                        if (count == 1) {
                            ...
                            all_combined[index1][index2] = group_wise[min_comp2][min_comp3].substring(0, index) + 'x'
                                    + group_wise[min_comp2][min_comp3].substring(index + 1, bits);

                            prime_fill(all_combined[index1][index2], group_wise[min_comp][min_comp1],
                ...
                while (prime_calc[prime_calc_index][check_index] != "-1") {
                    ...
                    if (prime_calc[prime_calc_index][current_index].equals(prime_calc[prime_calc_index][check_index])) {
                        ...
                        while (prime_calc[prime_calc_index][temp_check_index] != "-1") {
                            prime_calc[prime_calc_index][temp_check_index] =
